...
function renderLessonOutline(course) {
  if (!["A05", "A06", "A07"].includes(course.id)) return "";
...
function renderStaticContent(course) {
  const includeOutline = ["A05", "A06", "A07"].includes(course.id);
...
function renderCourseGoals(course) {
  if (!["A05", "A06", "A07"].includes(course.id) || !Array.isArray(course.goalArt) || course.goalArt.length === 0) {
