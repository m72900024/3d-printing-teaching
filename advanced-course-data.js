window.ADVANCED_COURSES = [
  ...
  {
    id:"A09", slug:"09-dimensional-tolerance.html", stage:"尺寸精度", stageNo:"C", duration:"45 分鐘", type:"量測＋裝配實作",
    title:"依裝配需求設計公差", subtitle:"先定義作品要怎麼配合，再用量測、補償與單一變因試片找到可重複的結果。",
    lead:"公差不是從網路抄一個間隙就結束。同一組尺寸會受機型、線材、方向、流量、底板與幾何影響。本課從滑動、定位、緊配與螺絲孔四種用途出發，分清名義尺寸和實際尺寸，再判斷應修改模型間隙、X-Y 內輪廓尺寸補償、X-Y 外輪廓尺寸補償，或只處理首層象腳。",
    goals:["依滑動、定位、緊配與螺絲孔需求描述配合結果","分辨 CAD 名義尺寸、列印實際尺寸與功能間隙","正確判讀 X-Y 內輪廓與外輪廓補償的正負方向","判斷首層干涉是否來自象腳而非整體尺寸","以單一變因公差試片建立自己的裝配紀錄"],
    goalArt:[{src:"../assets/advanced-a09/illustrations/fit-types.webp",alt:"滑動配合、定位配合、緊配合與螺絲孔四種裝配用途圖解"}],
    sections:[
      {
        title:"先說清楚：這組零件要怎麼配合",
        body:"先用動作和功能描述需求，不要先猜數字。滑動配合要能反覆移動且不明顯卡住；定位配合要容易插入並限制晃動；緊配合要靠壓入保持位置；螺絲孔則要分清通孔、自攻孔、螺紋孔或沉頭空間。用途不同，能接受的阻力、鬆動與拆卸方式也不同。",
        manga:{src:"../assets/advanced-a09/illustrations/fit-types.webp",alt:"軸套滑動、定位銷、壓入配合與螺絲通孔的功能對照",label:"GPT 教學圖解",caption:"同樣是軸與孔，目標可能是滑動、定位、固定或讓螺絲通過；先定義功能，才知道怎樣算成功。",guides:[
          {number:"1",title:"滑動配合",text:"可反覆推拉或旋轉，不能因局部高點明顯卡住。"},
          {number:"2",title:"定位配合",text:"能順利插入，組合後不出現影響功能的晃動。"},
          {number:"3",title:"緊配合",text:"需要受控壓力裝入，並由干涉保持位置；先用非安全關鍵小件測試。"},
          {number:"4",title:"螺絲孔",text:"先確認螺絲是自由通過、自攻成形，還是配合螺帽或金屬嵌件。"}
        ]},
        compareHeaders:["用途","合格的可觀察結果","先避免"],
        compare:[["滑動","可重複移動、不明顯偏擺","只看能否第一次塞入"],["定位","容易插入、晃動在需求內","把零間隙當成精準"],["緊配","受控壓入、組合後保持","用脆弱件徒手硬壓"],["螺絲孔","符合指定緊固方式","把通孔、自攻孔混為一談"]],
        callout:"裝配件承受力量時，尺寸合適不代表方向安全；固定點、受力與層間方向仍要回到 A05 檢查。",
        sources:[{label:"先修回顧：A05｜受力方向與列印方向",url:"05-force-direction.html"}]
      },
      {
        title:"名義尺寸、實際尺寸與功能間隙",
        body:"名義尺寸是 CAD 或圖面指定的目標；實際尺寸是冷卻後用相同方法量到的結果。功能間隙則是兩個配合面之間為了滑動、定位或壓入而刻意保留的差。先把量具歸零，讓零件完全冷卻，在左、中、右或 X、Y 多個位置輕觸量測；卡尺夾得太緊、只量一次或把孔徑與外徑混在一起，都會讓結論失真。",
        manga:{src:"../assets/advanced-a09/illustrations/nominal-vs-actual.webp",alt:"20.00 毫米名義尺寸與卡尺量得 19.72 毫米實際尺寸的偏差圖解",label:"GPT 教學圖解",caption:"偏差＝實際尺寸－名義尺寸。先記錄偏差發生在孔、外形還是首層，再決定工具。",guides:[
          {number:"1",title:"先冷卻再量",text:"剛離開平台的零件仍可能熱縮或受取件力量影響。"},
          {number:"2",title:"卡尺輕觸",text:"量測薄壁或首層時不要用力夾壓，避免把零件壓小。"},
          {number:"3",title:"至少量三處",text:"比較位置與方向，辨認是整體偏差、局部變形或橢圓。"},
          {number:"4",title:"保留正負號",text:"實際減名義後保留正負，才能判斷要放大還是縮小。"}
        ]},
        callout:"先用已知尺寸物確認量具讀值。一般游標卡尺不適合把深孔內徑當作高精度基準；課堂結果只用於目前作品和工具。"
      },
      {
        title:"孔與外形要分開補償",
        body:"Bambu Studio 的 X-Y 內輪廓尺寸補償調整閉合孔洞，正值會讓孔變大，負值會讓孔變小；X-Y 外輪廓尺寸補償調整物件外形，正值會讓外輪廓變大，負值會讓外輪廓變小。官方 Wiki 也提醒：孔的直徑變化是補償值的兩倍，因為孔壁兩側都會移動；側面開口造成走線未閉合時，內輪廓補償可能不適用。",
        manga:{src:"../assets/advanced-a09/illustrations/hole-contour-compensation.webp",alt:"X-Y 內輪廓和外輪廓補償的正負方向箭頭",label:"GPT 教學圖解",caption:"先圈出要改的是孔壁還是外形：正值都朝各自輪廓的放大方向，負值朝縮小方向。",guides:[
          {number:"1",title:"只改孔徑",text:"使用 X-Y 內輪廓尺寸補償，重新切片後確認閉合孔壁的移動方向。"},
          {number:"2",title:"只改外形",text:"使用 X-Y 外輪廓尺寸補償，檢查外壁和配合面的新位置。"},
          {number:"3",title:"注意直徑是兩側",text:"每側輪廓各移動一次，孔直徑變化會是設定補償值的兩倍。"},
          {number:"4",title:"開口幾何先看預覽",text:"若單層走線沒有形成閉合孔，不能假設內輪廓補償仍會照常作用。"}
        ]},
        callout:"不要同時修改模型尺寸、內輪廓補償與外輪廓補償。先選一個真正對應症狀的變因，保存 0 mm 基準再比較。",
        sources:[{label:"內容參考：Bambu Lab Wiki｜X-Y 內／外輪廓尺寸補償",url:"https://wiki.bambulab.com/zh/software/bambu-studio/xy-hole-contour-compensation"},{label:"功能依據：Bambu Studio 官方參數定義",url:"https://github.com/bambulab/BambuStudio/blob/master/src/libslic3r/PrintConfig.cpp"}]
      },
      {
        title:"只有底部卡住？先檢查象腳",
        body:"若零件上半部尺寸可接受，只有貼平台的最初幾層外擴並卡住配合面，問題更可能是象腳，不應先縮小整個外輪廓。象腳補償會在切片時主動內縮首層輪廓，用來抵消實際列印時的外擴。量測時比較底部與上方基準尺寸；不同底板和第一層狀態可能需要重新測試。",
        manga:{src:"../assets/advanced-a09/illustrations/elephant-foot.webp",alt:"象腳外擴、首層內縮補償與裝配恢復的三步圖解",label:"GPT 教學圖解",caption:"象腳補償只針對底部外擴；若整件作品都偏大，應回到外輪廓或製程診斷。",guides:[
          {number:"1",title:"比較底部與上方",text:"底部較寬、上方接近基準，才符合象腳候選。"},
          {number:"2",title:"先排除第一層異常",text:"平台選擇、髒污或不合理的首層壓縮也會影響底部。"},
          {number:"3",title:"只調象腳補償",text:"保留其他尺寸設定，從測得偏差建立一個候選值。"},
          {number:"4",title:"重新裝配驗證",text:"比較底部是否順利通過，同時確認首層仍連續且附著正常。"}
        ]},
        callout:"象腳補償不是改善平台附著的設定。數值過大可能讓首層邊緣內縮；調整後必須查看預覽並實際驗證。",
        sources:[{label:"內容參考：Bambu Lab Wiki｜象腳補償",url:"https://wiki.bambulab.com/zh/software/bambu-studio/parameter/elephant-foot"}]
      },
      {
        title:"用階梯試片找目前作品的可用間隙",
        body:"建立小型公差試片，在同一張板上放入多組只改間隙的孔軸或卡槽。固定模型方向、層高、線材預設、外殼、速度、底板與冷卻；完全冷卻後依序測試太緊、可定位、可滑動或太鬆。試片上的數字是幾何間隙，不是跨材料、跨方向都有效的保證。",
        manga:{src:"../assets/advanced-a09/illustrations/tolerance-coupon.webp",alt:"五段間隙公差試片由太緊到可定位、可滑動與太鬆的比較",label:"GPT 教學圖解",caption:"同一次列印只改間隙，快速找出目前機型、線材、方向與用途下的候選區間。",guides:[
          {number:"1",title:"保留 0 補償基準",text:"第一輪先不加切片補償，避免模型間隙和製程補償混在一起。"},
          {number:"2",title:"相同方向一起印",text:"同一板、同一方向和同一線材可減少批次差異。"},
          {number:"3",title:"依用途判讀",text:"滑動、定位與緊配的合格位置可能不同，不選唯一冠軍。"},
          {number:"4",title:"保存實體試片",text:"標記日期、材料、底板、噴嘴和製程預設，作為下次起點。"}
        ]},
        compareHeaders:["只改的變因","保持不變","記錄結果"],
        compare:[["幾何間隙","方向、材料、層高、外殼、速度、底板","太緊／定位／滑動／太鬆"],["下一輪才改補償","沿用已選定的試片與條件","補償前後量測與裝配差異"]],
        callout:"本課沒有通用公差值。網路上的間隙只能當試片範圍靈感，最後必須由自己的機型、材料、方向與功能測試決定。",
        sources:[{label:"官方實作流程：Bambu Lab Wiki｜X-Y 內／外輪廓尺寸補償",url:"https://wiki.bambulab.com/zh/software/bambu-studio/xy-hole-contour-compensation"}]
      },
      {
        title:"量測、記錄，再決定接受或調整",
        body:"完成試片後，把用途、名義尺寸、三處實際讀值、方向、材料、噴嘴、底板、製程預設和裝配手感放在同一張紀錄表。若結果符合功能就接受；若只有孔、外形或首層出現一致偏差，再只調對應補償；若偏差忽大忽小、孔呈橢圓或表面品質異常，先回到診斷，不要用補償掩蓋不穩定製程。",
        manga:{src:"../assets/advanced-a09/illustrations/measure-record-decide.webp",alt:"選用途、看名義尺寸、列印試片、量實際尺寸與記錄決定的五步流程",label:"GPT 教學圖解",caption:"每一輪都留下可追溯證據：接受、只調一個變因，或回到診斷。",guides:[
          {number:"1",title:"接受",text:"尺寸與裝配行為符合需求，不因追求整齊數字繼續調整。"},
          {number:"2",title:"調整一個變因",text:"孔、外形或象腳哪裡出現一致偏差，就只處理那一項。"},
          {number:"3",title:"回到診斷",text:"結果不一致時先檢查受潮、出料、方向、翹曲與量測方法。"},
          {number:"4",title:"限制使用範圍",text:"把結果綁定目前材料、方向和用途；條件改變就先以小件重驗。"}
        ]},
        callout:"緊配、卡扣或螺絲測試可能突然斷裂或彈出。使用小型非安全關鍵試片、護目鏡和治具，不徒手朝臉部硬壓。",
        sources:[{label:"問題診斷回顧：A02｜品質問題診斷",url:"02-quality-diagnostics.html"},{label:"材料基準回顧：A08｜流量校正與建立線材預設",url:"08-flow-calibration.html"}]
      }
    ],
    task:{title:"完成一張裝配公差紀錄表",text:"從滑動、定位、緊配或螺絲孔選一種用途，建立只改間隙的五段小試片。使用同一方向、材料、製程與底板列印；完全冷卻後量測三處並實際裝配，標記太緊、合用或太鬆。若有一致偏差，第二輪只選 X-Y 內輪廓、外輪廓或象腳補償其中一項比較，保存補償前後照片與讀值。"},
    checkpoint:"我能先定義裝配用途，分清名義尺寸、實際尺寸與功能間隙，正確判讀孔、外形和象腳補償，並用單一變因試片建立自己的紀錄，而不是套用通用公差值。"
  }
];
